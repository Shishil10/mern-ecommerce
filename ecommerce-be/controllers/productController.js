...
export const createProductController = async (req, res) => {
  try {
    ...
    const products = new productModel({ ...req.fields, slug: slugify(name), isFeatured: req.fields.isFeatured || false, });
    if (photo) {
...
export const updateProductController = async (req, res) => {
  try {
    ...
    const products = await productModel.findByIdAndUpdate(
      req.params.pid,
      { ...req.fields, slug: slugify(name), isFeatured: isFeatured || false },
      { new: true }
    );
    if (photo) {
      products.photo.data = fs.readFileSync(photo.path);
